...
if [[ "$PRESCAN_OUT" == "CLEAR" ]]; then
    ...
    python3 -c "
import json, sys
marker = {'date': '$TODAY', 'project': 'daily-ai-news', 'status': 'done', 'diff_stat': '', 'summary': sys.stdin.read().strip()}
...
你正在对 Daily AI News wiki 进行每日自检和健康检查。
...
2. 扫描 wiki/topics/、wiki/sources/、wiki/timelines/、wiki/entities/、wiki/ideas/ 查找未在 index.md 中的 .md 文件，添加它们。
3. 检查 entity 页面：有没有公司或人物放在 entities/ 里？公司放 sources/，人物直接删除。

## 第三步 — 错误与重复信息
1. 查找覆盖相同概念的重复页面（如同一模型的两个 entity 页面）。合并它们。
2. 检查页面内重复的段落或章节。
3. 检查页面之间不一致的事实信息（如模型发布日期不同）并修正。
4. 检查 frontmatter 的日期、slug、type 是否一致正确。

## 第四步 — 连接质量
1. 找出只写 "相关：" 或 "参见：" 而没有标注的链接，重写说明 WHY。
2. 找出在 topics/sources 中提到但没有自己页面的 entity，创建页面。
3. 检查 topic 页面是否缺少 ## Evolution 章节，补写时间线叙述。
4. 检查 topic 页面是否缺少 ## Patterns & Insights，从事件中综合洞察。
