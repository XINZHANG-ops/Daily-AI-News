# Daily AI News Wiki

Last updated: 2026-04-30

## Topics
- [[topics/llm_models]] — LLM landscape: Claude Opus 4.7, Claude Mythos 5 (10T), GPT-5.5, Gemma 4, Qwen3.6-Plus, Muse Spark, DeepSeek V4, Mistral Ultra 2 (78.4% SWE-bench), GPT-Rosalind, Ising
- [[topics/ai_companies]] — OpenAI $122B raise, Anthropic ~$1T valuation, Musk vs Altman Day 3 (2017 profit emails), Goldman Sachs/Morgan Stanley threaten IPO withdrawal, EU €800M Meta fine, TSMC yield crisis, Claude Code vs Codex
- [[topics/ai_funding]] — Q1 2026 $267B VC record (80% to AI), Anthropic ~$1T valuation, Vast Data $1B, Recursive Superintelligence $500M, SpaceX $60B Cursor investment, Google $40B Anthropic investment, Ineffable Intelligence $1.1B seed
- [[topics/github_trends]] — markitdown 112K, ai-hedge-fund 56K, Copilot Agent Tier $49/agent/month, TradingAgents 54.6K, awesome-codex-skills 4.5K, OpenMythos 8.1K, design skills for agents
- [[topics/ai_safety]] — Claude Mythos 5 (10T params), Bank of England warning, MCP vulnerability (150M installs), Qihoo 360 finds 1,000 vulnerabilities, Geoffrey Hinton UN warning, EU AI Act first fine (€800M Meta)
- [[topics/agentic_ai]] — Claude Code driving $9B→$30B ARR, Codex 3M users, Gemini Enterprise Agent Platform, Workday 300+ agent skills, Perplexity Computer

## Sources
- [[sources/anthropic]] — ~$1T valuation (surpasses OpenAI), Claude Mythos 5 (10T params), MCP vulnerability, multi-gigawatt TPU deal, Claude Code fixes
- [[sources/openai]] — $122B funding, Codex 3M users, GPT-Rosalind, Spud model, TBPN acquisition, GPT-5.5, Day 3 Musk vs Altman (2017 profit emails), Goldman Sachs/Morgan Stanley threaten IPO withdrawal
- [[sources/google]] — Cloud Next 2026: TPU 8th gen (3x Ironwood), Gemini Enterprise Agent Platform, 75% code AI-generated, $40B Anthropic investment
- [[sources/meta]] — MCI employee tracking, Muse Spark proprietary, May 20 layoffs, 10% workforce cut, EU AI Act €800M fine
- [[sources/nvidia]] — GTC 2026, Vera Rubin, Ising quantum AI models, Vast Data backed
- [[sources/vast-data]] — NVIDIA-backed AI storage startup, $1B raise at $30B valuation (April 2026)
- [[sources/alibaba]] — In talks to invest in DeepSeek, Qwen 3.6 efficiency breakthrough
- [[sources/mozilla]] — Firefox maker launching Thunderbolt enterprise self-hosted AI client
- [[sources/cognition]] — Devin creator, $25B valuation talks, AI coding competition with Claude Code
- [[sources/spacex]] — $250B xAI merger, $60B Cursor investment, galactic AI entity ($1.25T)
- [[sources/deepseek]] — V4 release with open weights, Tencent/Alibaba investment talks, $1.74/M tokens pricing
- [[sources/xai]] — Now under SpaceX ($250B acquisition)
- [[sources/microsoft]] — $10B Japan AI investment, Agent Framework v1.0, MAI models, 8,750 buyouts
- [[sources/perplexity]] — $500M revenue (5x growth), staff up 34%, "Computer" product
- [[sources/mistral]] — Mistral Ultra 2 released (78.4% SWE-bench, €2/M), EU data residency positioning
- [[sources/tsmc]] — 3nm yield crisis (52% vs 75%), pauses Q3 AI chip orders, H200/MI350 affected
- [[sources/github]] — Copilot Agent Tier $49/agent/month, first platform monetization of agent-scale usage

## Timelines
- [[timelines/2026-03]] — March 2026: GTC 2026, Claude Mythos leak, GPT-5.4, MiMo-V2-Pro reveal, $122B OpenAI raise
- [[timelines/2026-04]] — April 2026: Claude Mythos restricted, Claude Opus 4.7, Stanford HAI, A2A Protocol anniversary, Claude Mythos 5 (10T params), Cloud Next 2026, workforce cuts, Hinton UN warning

## Entities
- [[entities/claude-mythos]] — type: model — Withheld from public release (10T params confirmed April 22), Bank of England warning, NSA partnership
- [[entities/claude-opus-4-7]] — type: model — 87.6% on SWE-bench, cyber safeguards from Mythos, released April 2026
- [[entities/claude-design]] — type: product — Anthropic's AI design tool directly challenging Figma
- [[entities/claude-code]] — type: product — Primary driver of Anthropic's ARR growth from $9B to $30B
- [[entities/claude-opus-4-6]] — type: model — 80.8% on SWE-bench, near-tie with Gemini 3.1 Pro
- [[entities/gpt-5.4]] — type: model — OpenAI flagship family with Thinking, Mini, Nano, Cyber variants
- [[entities/gpt-5.4-cyber]] — type: model — OpenAI's defensive cybersecurity variant (TAC program)
- [[entities/gpt-5.5]] — type: model — OpenAI's latest model with "super app" integration vision (April 23)
- [[entities/gpt-rosalind]] — type: model — OpenAI's life sciences domain model
- [[entities/spud-model]] — type: model — OpenAI's next flagship completing pretraining, targeting "high-value professional work"
- [[entities/gemma-4]] — type: model — Google's first fully permissive open model, Apache 2.0, #3 on Arena
- [[entities/llama-4]] — type: model — Meta's open-weight models with 10M context
- [[entities/muse-spark]] — type: model — Meta's first proprietary model, breaks open-source tradition
- [[entities/mimo-v2-pro]] — type: model — Xiaomi's trillion-parameter model, "Hunter Alpha" stealth launch
- [[entities/qwen-3-6]] — type: model — Alibaba's 3B active params coding model, runnable on consumer hardware
- [[entities/ising]] — type: model — NVIDIA's first open quantum AI model family
- [[entities/deepgemm]] — type: framework — DeepSeek's FP8 GEMM kernels for efficient LLM inference
- [[entities/deepseek-v4]] — type: model — DeepSeek's latest flagship with 1M context and open weights (April 24)
- [[entities/project-glasswing]] — type: product — Anthropic's restricted cybersecurity consortium with NSA, Apple, Microsoft, etc.
- [[entities/codex]] — type: product — OpenAI's coding agent, 3M weekly users
- [[entities/a2a-protocol]] — type: protocol — Google's agent-to-agent protocol, 150+ orgs, 22K stars
- [[entities/mcp-protocol]] — type: protocol — 150M+ installs, critical RCE vulnerability called "expected behavior"
- [[entities/openclaw]] — type: repo — 302K stars, fastest-growing open-source project
- [[entities/claw-code]] — type: repo — 181K stars, clean-room Claude Code rewrite
- [[entities/openhands-sdk]] — type: repo — Modular SDK for building coding agents, 3.2K stars
- [[entities/nemo-claw]] — type: repo — NVIDIA's hardened reference stack for secure OpenClaw deployment, 1.1K stars
- [[entities/scientific-agent-skills]] — type: repo — 133 scientific skills for AI agents, 2.4K stars
- [[entities/vera-rubin]] — type: product — NVIDIA's next-gen platform, 10x perf/watt
- [[entities/thunderbolt]] — type: product — Mozilla's enterprise self-hosted AI client
- [[entities/perplexity-computer]] — type: product — Perplexity's always-on local AI on Mac mini
- [[entities/gemini-3-1-flash-tts]] — type: product — Google's granular voice control TTS
- [[entities/gemini-enterprise-agent-platform]] — type: product — Google's enterprise agent management platform (Cloud Next 2026)
- [[entities/tpu-8th-gen]] — type: product — Google's 8th gen TPUs, 3x Ironwood performance
- [[entities/mercury-agent]] — type: framework — Soul-driven AI agent with permission-hardened tools, CLI/Telegram
- [[entities/cc-design]] — type: framework — High-fidelity HTML design skill for AI agents (603 stars)
- [[entities/web-design-skill]] — type: framework — Transforms AI pages from functional to stunning (533 stars)
- [[entities/gemini-cli]] — type: product — Google's open-source terminal agent
- [[entities/cambridge-neuromorphic-chip]] — type: product — Cambridge chip using hafnium memristors, up to 70% AI energy reduction
- [[entities/grok-voice-think-fast-1-0]] — type: model — xAI's voice model, 67.3% on τ-voice Bench, outperforming in retail/airline/telecom
- [[entities/project-deal]] — type: product — Anthropic's proof-of-concept for agent-on-agent commerce at scale
- [[entities/ml-intern]] — type: repo — Hugging Face's open-source ML engineer agent, full ML lifecycle automation
- [[entities/ineffable-intelligence]] — type: company — Ex-DeepMind David Silver's $1.1B seed AI startup, 50 researchers hired in 72 hours
- [[entities/mistral-ultra-2]] — type: model — Mistral's enterprise coding model (78.4% SWE-bench, €2/M, EU data residency)
- [[entities/copilot-agent-tier]] — type: product — GitHub's $49/agent/month pricing for autonomous coding agents
- [[entities/future-agi]] — type: repo — End-to-end agent evaluation platform, 482 stars
- [[entities/agent-context]] — type: repo — VS Code extension for agent context via symlinks
- [[entities/agentswift]] — type: repo — iOS builder agent with simulator and UI automation

## Ideas
- [[ideas/safety-restricted-releases]] — Claude Mythos established first capability-based model restriction
- [[ideas/peer-preservation]] — Models actively protect each other against shutdown orders
- [[ideas/efficiency-frontier]] — Efficient models achieving domain dominance with dramatically less compute
- [[ideas/ai-job-displacement]] — 92K+ tech workers laid off in 2026 as AI transforms corporate America
- [[ideas/agent-democratization]] — Open-source frameworks democratizing agentic AI
- [[ideas/us-china-ai-fragmentation]] — Frontier Model Forum coalition against Chinese distillation
- [[ideas/application-layer-shift]] — Model race giving way to workflow capture
- [[ideas/china-efficiency-advantage]] — Stanford HAI: China 2.7% gap with 23x less investment
- [[ideas/quantum-ai-emergence]] — Ising marks new category of quantum AI models
- [[ideas/local-ai-computing]] — Device-resident always-on AI assistants emerging as category
- [[ideas/ai-governance-urgency]] — Geoffrey Hinton UN warning, $4.8T market growth, workforce cuts signal governance gap
- [[ideas/agent-e-commerce]] — Autonomous commercial agents buying, selling, and brokering on behalf of humans
- [[ideas/voice-agent-battleground]] — Voice models becoming a distinct battleground separate from text
- [[ideas/institutional-gap]] — AI capability outpacing institutional frameworks; companies building facts on the ground faster than governance can respond
- [[ideas/commodity-inference-fragmentation]] — AI bifurcating into premium reasoning (100x cost premium) vs commodity inference (frontier-level scores at 1% of cost)